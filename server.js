...
// --- KONFIGURACE ZÁLOHOVÁNÍ NA GITHUB ---
const GITHUB_TOKEN = process.env.GITHUB_TOKEN || 'TVUJ_GITHUB_TOKEN'; // Doplň svůj GitHub token (s oprávněním zapisovat do repozitáře)
const GITHUB_REPO = process.env.GITHUB_REPO || 'uzivatel/repozitar'; // např. 'matej/neo-survivor'
const GITHUB_FILE_PATH = 'db_backup/neo_survivor_backup.enc';
const ENCRYPTION_KEY = process.env.BACKUP_PASSWORD || '12345678901234567890123456789012'; // Musí mít přesně 32 znaků!
...
// ZÁLOHOVÁNÍ NA GITHUB (ŠIFROVANĚ)
async function backupDatabaseToGithub() {
    try {
        if (GITHUB_TOKEN === 'TVUJ_GITHUB_TOKEN') {
            console.log("ZÁLOHA ZRUŠENA: Nebyl nastaven GITHUB_TOKEN. Nastav jej v proměnných prostředí nebo přímo v kódu.");
            return;
        }
        
        const dbPath = './neo_survivor.db';
        if (!fs.existsSync(dbPath)) return;
        
        const fileBuffer = fs.readFileSync(dbPath);
        
        // AES-256-CBC šifrování
        const iv = crypto.randomBytes(16);
        const cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(ENCRYPTION_KEY.padEnd(32, '0').slice(0, 32)), iv);
        let encrypted = cipher.update(fileBuffer);
        encrypted = Buffer.concat([encrypted, cipher.final()]);
        
        const backupData = iv.toString('hex') + ':' + encrypted.toString('hex');
        const contentBase64 = Buffer.from(backupData).toString('base64');

        const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${GITHUB_FILE_PATH}`;
        
        let sha = null;
        try {
            const getRes = await fetch(url, { headers: { 'Authorization': `Bearer ${GITHUB_TOKEN}`, 'User-Agent': 'NeoSurvivor-Server' } });
            if (getRes.ok) {
        ...
        const body = {
            message: `Automatická záloha databáze ${new Date().toISOString()}`,
        ...
        const putRes = await fetch(url, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${GITHUB_TOKEN}`, 'Content-Type': 'application/json', 'User-Agent': 'NeoSurvivor-Server' },
            body: JSON.stringify(body)
        });

        if (putRes.ok) {
            console.log(`[ZÁLOHA ÚSPĚŠNÁ] Databáze zazálohována na GitHub (${new Date().toLocaleString()})`);
        } else {
            const errData = await putRes.json();
            console.error("[CHYBA ZÁLOHY] Nepodařilo se nahrát na GitHub:", errData.message);
        }
    } catch (err) {
        console.error("[CHYBA ZÁLOHY] Proces selhal:", err.message);
    }
}

// Spouštění zálohy každou hodinu (3600000 ms)
setInterval(backupDatabaseToGithub, 3600000);

// SERVEROVÁ KONZOLE - PŘÍKAZY
...
rl.on('line', (input) => {
    ...
    if (cmd === 'info') {
        ...
        if (!username) return console.log("Použití: info <jméno>");
        db.get(`SELECT * FROM accounts WHERE username = ?`, [username], (err, row) => {
            if (row) {
                console.log(`\n--- STATISTIKY HRÁČE: ${username} ---`);
                console.log(`Max Level: ${row.max_level}`);
                console.log(`Data (Meta):`, JSON.stringify(JSON.parse(row.meta), null, 2));
                console.log(`------------------------------------\n`);
            } else {
                console.log("Hráč nenalezen v databázi.");
            }
        });
    } else if (cmd === 'givecurrency' || cmd === 'givedoge') {
        const username = args[0];
        const amount = parseInt(args[1]);
        if (!username || isNaN(amount)) return console.log("Použití: givecurrency <jméno> <množství>");
        
        db.get(`SELECT meta FROM accounts WHERE username = ?`, [username], (err, row) => {
            if (row) {
                const meta = JSON.parse(row.meta);
                meta.currency = (meta.currency || 0) + amount;
                db.run(`UPDATE accounts SET meta = ? WHERE username = ?`, [JSON.stringify(meta), username], () => {
                    console.log(`[ADMIN] Hráči ${username} bylo přidáno ${amount} DOGE. Nový zůstatek: ${meta.currency}`);
                });
            } else {
                console.log("Hráč nenalezen.");
            }
        });
    } else if (cmd === 'setlevel') {
        const username = args[0];
        const level = parseInt(args[1]);
        if (!username || isNaN(level)) return console.log("Použití: setlevel <jméno> <level>");
        
        db.get(`SELECT meta FROM accounts WHERE username = ?`, [username], (err, row) => {
            if (row) {
                const meta = JSON.parse(row.meta);
                meta.maxLevel = level;
                db.run(`UPDATE accounts SET meta = ?, max_level = ? WHERE username = ?`, [JSON.stringify(meta), level, username], () => {
                    console.log(`[ADMIN] Hráči ${username} byl nastaven level na ${level}.`);
        ...
        // giveitem <jmeno> <lod/schopnost> <id>
        const username = args[0];
        const type = args[1]; // 'ship' nebo 'ability'
        const id = args[2];
        if (!username || !type || !id) return console.log("Použití: giveitem <jméno> <ship/ability> <id_číslo>");
        ...
        db.get(`SELECT meta FROM accounts WHERE username = ?`, [username], (err, row) => {
            if (row) {
                ...
                if (type === 'ship') {
                    ...
                    meta.ships[id] = true;
                    console.log(`[ADMIN] Hráči ${username} byla přidána loď ID ${id}.`);
                } else if (type === 'ability') {
                    if(!meta.abilities) meta.abilities = { 1: true };
                    meta.abilities[id] = true;
                    console.log(`[ADMIN] Hráči ${username} byla přidána schopnost ID ${id}.`);
                }
                db.run(`UPDATE accounts SET meta = ? WHERE username = ?`, [JSON.stringify(meta), username]);
            } else {
                console.log("Hráč nenalezen.");
        ...
        console.log("Spouštím manuální zálohu databáze...");
        backupDatabaseToGithub();
    } else if (cmd === 'help') {
        console.log("Dostupné příkazy:");
        console.log("  info <jméno>                       - Vypíše statistiky hráče");
        console.log("  givecurrency <jméno> <množství>    - Přidá hráči zadaný počet DOGE");
        console.log("  setlevel <jméno> <level>           - Nastaví hráči maximální level");
        console.log("  giveitem <jméno> <ship/ability> <id> - Odblokuje hráči loď nebo schopnost");
        console.log("  backup                             - Vynutí okamžitou zálohu DB na GitHub");
    } else {
        console.log("Neznámý příkaz. Napiš 'help' pro seznam příkazů.");
    }
});
                ...
                if (enemy.type === 2) {
                    ...
                    if (now - enemy.lastShot > dynamicInterval) {
                        ...
                        io.to(roomId).emit('enemyShoot', {
                            ...
                            dmg: 10, speed: 10, size: 8, type: 'default' 
                        });
                        enemy.lastShot = now;
                    }
                }
