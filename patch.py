...
enLines = ',\n'.join([f'            "{k}": "{v[0]}"' for k,v in upgrades.items()])
deLines = ',\n'.join([f'            "{k}": "{v[1]}"' for k,v in upgrades.items()])
esLines = ',\n'.join([f'            "{k}": "{v[2]}"' for k,v in upgrades.items()])

content = re.sub(r'("ZRUŠIT": "CANCEL")\n        },', r'\1,\n' + enLines + '\n        },', content)
content = re.sub(r'("ZRUŠIT": "ABBRECHEN")\n        },', r'\1,\n' + deLines + '\n        },', content)
content = re.sub(r'("ZRUŠIT": "CANCELAR")\n        }', r'\1,\n' + esLines + '\n        }', content)
